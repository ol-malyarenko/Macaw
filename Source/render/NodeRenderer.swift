...
class NodeRenderer {
    ...
    func renderToImage(bounds: Rect, transform: Rect, rotation: Double, coloringMode: ColoringMode = .rgb) -> MImage {
        let screenScale: CGFloat = MMainScreen()?.mScale ?? 1.0
        ...
        switch rotation {
        case 90, -90:
            tempContext.translateBy(x: -CGFloat(transform.y), y: CGFloat(bounds.h + transform.x / 2))
            tempContext.scaleBy(x: CGFloat(transform.h), y: -CGFloat(transform.w))
        default:
            tempContext.translateBy(x: -CGFloat(transform.x), y: CGFloat(bounds.h + transform.y / 2))
